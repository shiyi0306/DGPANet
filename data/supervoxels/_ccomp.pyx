...
# so for our example, array[2] == 1, array[4] == 2 and array[1] == 1, because
# 1 is the root.
...
# When we found out that label 2 and 3 are the same, we assign array[2] = 3.

cdef DTYPE_t find_root(DTYPE_t *forest, DTYPE_t n) nogil:
    """Find the root of node n.
    Given the example above, for any integer from 1 to 3, 1 is always returned
    """
    cdef DTYPE_t root = n
...
cdef inline void set_root(DTYPE_t *forest, DTYPE_t n, DTYPE_t root) nogil:
    ...
    Given the example above, given n=3, root=2, it would "reconnect" the tree.
    so forest[3] = 2 and forest[4] = 2
...
cdef inline void join_trees(DTYPE_t *forest, DTYPE_t n, DTYPE_t m) nogil:
    ...
    rather have two disjoint trees with roots 2 and 2.
    Joining them would mean that all elements of both trees become connected
    to the element 2, so forest[3] == 2, forest[2] == 2 etc.
