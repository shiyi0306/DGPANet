...
def main(_run, _config, _log):
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            # query_loss_ = criterion(torch.log(torch.clamp(query_pred_, torch.finfo(torch.float32).eps,
            #                                              1 - torch.finfo(torch.float32).eps)), query_labels)
            # query_loss = 0.7 * query_loss + 0.3 * query_loss_
            # query_loss = query_loss + query_loss_
            # align_loss = 0.5 * align_loss
